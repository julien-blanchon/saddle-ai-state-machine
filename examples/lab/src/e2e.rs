use bevy::prelude::*;
use saddle_ai_state_machine::AiStateMachineSystems;
use saddle_bevy_e2e::action::Action;

use crate::scenarios;

pub struct StateMachineLabE2EPlugin;

impl Plugin for StateMachineLabE2EPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(saddle_bevy_e2e::E2EPlugin);
        app.configure_sets(
            Update,
            saddle_bevy_e2e::E2ESet.before(AiStateMachineSystems::IntakeSignals),
        );
        ...
        if let Some(name) = scenario_name {
            if let Some(mut scenario) = scenarios::scenario_by_name(&name) {
                ...
                error!(
                    "[sm_lab:e2e] Unknown scenario '{name}'. Available: {:?}",
...
fn parse_e2e_args(args: &[String]) -> (Option<String>, bool) {
    ...
    if !handoff {
        handoff = std::env::var("E2E_HANDOFF").is_ok_and(|v| v == "1" || v == "true");
    }
